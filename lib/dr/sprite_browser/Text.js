...
define(function(require, exports, module) {
    ...
    module.exports = sprite.Text = new JS.Class('sprite.Text', require('./View.js'), {
        ...
        set_text: function(v) {
            if (v != null) {
                ...
                if (v !== this.getText(true)) {
                    ...
                    if (len === 1) {
                        // Optimize update for simple case
                        childNodes[0].data = v;
                    } else {
                        var i = 0, child;
                        for (; len > i;) {
                            child = childNodes[i++];
                            if (child.nodeType === 3) platformObject.removeChild(child);
                        }
        ...
        getText: function(noTrim) {
            ...
            var childNodes = this.platformObject.childNodes,
                len = childNodes.length;
            if (len === 1) {
                // Optimize for simple case
                return noTrim ? childNodes[0].data : childNodes[0].data.trim()
            } else {
                var texts = [], i = 0, child;
                for (; len > i;) {
                    child = childNodes[i++];
