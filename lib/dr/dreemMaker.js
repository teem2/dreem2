      ...
      function walk(node, parent, language) {
        ...
        switch (tagName) {
          ...
          case 'setter':
            ...
            break;
          case 'replicator':
            // Try to make a class out of the replicator body
            if (!attr.classname) {
              var generatedClassname = '__REPLICATOR_CLASS_' + dr.generateGuid();
              
              // Find the "template" and remove it.
              var templateNode, childTagName;
              children = node.child;
              if (children) {
                i = children.length;
                while (i) {
                  ...
                  if (
                    childTagName !== 'method' && childTagName !== 'handler' && 
                    childTagName !== 'attribute' && childTagName !== 'setter' &&
                    childTagName !== 'class' && childTagName !== 'mixin' && // class and mixin are possibly overkill.
                    !childTagName.startsWith('$')
                  ) {
              ...
              // Convert the template into a class
              if (templateNode) {
                if (!templateNode.attr) templateNode.attr = {};
                templateNode.attr.extends = templateNode.tag;
                templateNode.attr.name = generatedClassname;
                templateNode.tag = 'class';
                
                // And move walk it immediately
                walk(templateNode, node, language);
                
                attr.classname = generatedClassname;
              }
            }
        ...
        for (; i < len; i++) {
          ...
          if (compiled instanceof maker.Error) {
            ...
            compiled = compiled.split(
                'this["super"](').join('this.callSuper(').split(
                'this.super(').join('this.callSuper(').split(
                'this.super)').join('this.callSuper)').split(
                'this["super"] ').join('this.callSuper ');
