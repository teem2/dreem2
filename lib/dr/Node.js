    ...
    module.exports = dr.Node = new JS.Class('Node', {
        ...
        extend: {
            ...
            /** Creates a new child node of this node.
                @param parent:object The default parent to make the new child
                    on. Can be overridden by the parent property of the attrs.
                ...
                @param augmentor:function (optional) A function that gets called
                    with each child/descendant created.
                @returns The new child or undefined if the child could not
                    be created. */
            createChild: function(parent, attrs, mixins, augmentor) {
                var classname = 'node', klass;
                ...
                if (attrs) {
                    ...
                    if (attrs.parent) {
                        parent = attrs.parent;
                        if (parent === 'screen') parent = undefined;
                        delete attrs.parent;
                    }
                } else {
                    attrs = {};
                    ...
                    if (children) {
                        ...
                        mixins.push({
                            doBeforeAdoption: function() {
                                for (var i = 0, len = children.length; len > i; i++) dr.Node.createChild(this, children[i], null, augmentor);
                                this.callSuper();
                            }
                        });
                    }
                    
                    var instance = new klass(parent, attrs, mixins);
                    if (augmentor) augmentor(instance);
                    return instance;
                } else {
        ...
        destroy: function() {
            if (!this.isBeingDestroyed && !this.destroyed) {
        ...
        set_parent: function(newParent) {
            ...
            if (this.parent !== newParent) {
                ...
                // Abort if new parent is a leaf
                if (newParent && newParent.isLeaf(this)) return;
        ...
        set_name: function(name) {
            if (this.name !== name) {
                ...
                this.name = name;
                if (this.initing === false) this.sendEvent('onname', name);
        ...
        /** Creates a new child node of this node.
            ...
            @returns The new child or undefined if the child could not
                be created. */
        createChild: function(attrs, mixins, augmentor) {
            var instance = dr.Node.createChild(this, attrs, mixins, augmentor);
            
            // Execute notifyReady on every node and also update layouts
            (function callNotifyReady(node) {
                var sns = node.getSubnodes(), len = sns.length, i = 0;
                for (; len > i; i++) callNotifyReady(sns[i]);
                node.notifyReady();
                
                // Also update layouts since they won't be ready by the time
                // initNode completes.
                if (node.isA(dr.Layout)) node.update();
            })(instance);
        ...
        /** Use this method to check if a node supports having subnodes or not.
            @param subnode:Node The child node that is trying to add itself
                to this node.
            @returns boolean True if subnodes are not allowed, false 
                otherwise. Returns false by default. */
        isLeaf: function(subnode) {
            return false;
        },
        ...
        /** Walks the node tree depth first searching for the first node
            for which the matcher function returns true.
            @param matcherFunc:function A function with a single argument which
                is this node. Should return true if the node is a match.
            @param accum:array (optional) An accumulator for matches. If 
                provided matches will be pushed into the accumulator and
                searching will not terminate on the first match.
            @returns A dr.Node if a match is found or undefined if no match
                is found or an accumulator was provided. */
        search: function(matcherFunc, accum) {
            if (matcherFunc(this)) {
                if (accum) {
                    accum.push(this);
                } else {
            ...
            var subnodes = this.getSubnodes(), len = subnodes.length, i = 0, match;
            for (; len > i;) {
                match = subnodes[i++].search(matcherFunc, accum);
