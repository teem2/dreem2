	...
	dalihost.init = function(){
		...
		this.CreateShader = function(dotted)
		{
			...
			{                  \
				...
				lowp float inside = udRoundBox(pixpos, borderwidth, borderwidth, (width-borderwidth*2.0  ) , height-borderwidth*2.0,max(0, radius-borderwidth )) ;"  
		...
		this.CreateView = function(x,y,width,height) 
		{
			...
			var paddingcontainer = new dali.ImageActor();

			//act.setImage(this.defaultimage);
			...
			paddingcontainer.position = [paddingbottom + borderwidth,paddingtop  +borderwidth,0.6];
			paddingcontainer.size = [ width - paddingleft - paddingright - borderwidth * 2, height - paddingtop - paddingbottom - borderwidth * 2, 1];
			...
			rect.position = [width/2,height/2,1];	  
...
//			console.log(paddingcontainer.drawMode);
		...
		//		bounds.bottom = 40;
			...
			R.setSize = function(width, height)	{	
				...
				this.rectactor.position = [width/2, height/2,0.6];
				if (this.textactor)
				{
