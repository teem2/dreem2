...
define(function(require, exports, module) {
    ...
    module.exports = dr.Observable = new JS.Module('Observable', {
        ...
        /** Generates a new event from the provided type and value and fires it
            to the registered observers after a delay. If sendCollapsedEvent
            method is called for the same type before the previous delayed 
            event has fired the previous event will be cancelled.
            @param type:string The event type to fire.
            @param value:* The value to set on the event.
            @param delay:number (optional) The millis to delay by before 
                sending the event. If not provided 0 will be used.
            @param immediate:boolean (optional) If true is provided then an
                event will be sent immediately but any subsequent calls to
                sendCollapsedEvent within the delay millis for the same type 
                will be delayed.
            @returns This object for chainability. */
        sendCollapsedEvent: function(type, value, delay, immediate) {
            var self = this,
                timerName = '__' + type + '_timerId';
            ...
            this[timerName] = setTimeout(function() {
                if (!immediate && !this.destroyed) self.sendEvent(type, value);
                delete self[timerName];
            }, delay || 0);
            
            if (immediate) this.sendEvent(type, value);
