...
define(function(require, exports, module){
    ...
    dr.Observer = new JS.Module('Observer', {
        ...
        syncTo: function(observable, eventType, methodName, attrName, once) {
            ...
            try {
                var event = observable.getAttribute(attrName);
        ...
        listenTo: function(observable, eventTypes, methodName, once) {
            if (observable && methodName && eventTypes) {
                ...
                var len = eventTypes.length, i = 0, eventType, observables;
                ...
                if (once) {
                    ...
                    this[methodName] = function(event) {
                        self.stopListening(observable, eventTypes, methodName);
        ...
        stopListening: function(observable, eventTypes, methodName) {
            if (observable && methodName && eventTypes) {
                ...
                if (observablesByType) {
                    eventTypes = eventTypes.split(/[ ,]+/);
                    var len = eventTypes.length, i = 0, j, eventType, observables;
                    for (; len > i;) {
                        eventType = eventTypes[i++];
                        if (eventType) {
