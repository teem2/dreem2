...
/**
  ...
  *
  * The visual base class for everything in dreem. Views extend dr.node 
  * to add the ability to set and animate visual attributes, and interact 
  * with the mouse.
  *
  * Views are positioned inside their parent according to their x and 
  * y coordinates.
  *
  * Views can contain methods, handlers, setters, constraints, attributes 
  * and other view, node or class instances.
  *
  * Views can be easily converted to reusable classes/tags by changing their 
  * outermost &lt;view> tags to &lt;class> and adding a name attribute.
  *
  * Views support a number of builtin attributes. Setting attributes that 
  * aren't listed explicitly will pass through to the underlying Sprite 
  * implementation.
  *
  * Note that dreem apps must be contained inside a top-level &lt;view>&lt;/view> tag.
  *
  * The following example shows a pink view that contains a smaller blue view 
  * offset 10 pixels from the top and 10 from the left.
  *
  *     @example
  ...
  *     </view>
  *
  * Here the blue view is wider than its parent pink view, and because the 
  * clip attribute of the parent is set to false it extends beyond the 
  * parents bounds.
  *
  *     @example
  ...
  *     </view>
  *
  * Now we set the clip attribute on the parent view to true, causing the 
  * overflowing child view to be clipped at its parent's boundary.
  *
  *     @example
  ...
  *     </view>
  *
  * Here we demonstrate how unsupported attributes are passed to the 
  * underlying sprite system. We make the child view semi-transparent by 
  * setting opacity. Although this is not in the list of supported attributes 
  * it is still applied.
  *
  *     @example
  *     <view width="200" height="100" bgcolor="lightpink">
  ...
  *     </view>
  *
  * It is convenient to [constrain](#!/guide/constraints) a view's size and 
  * position to attributes of its parent view. Here we'll position the inner 
  * view so that its inset by 10 pixels in its parent.
  *
  *     @example
  *     <view width="200" height="100" bgcolor="lightpink">
...
/**
  ...
  * moveBackward, moveInFrontOf or moveBehind actually changes the subview order.
  * @param {dr.View} subview The dr.View that was reordered.
  */

/**
  * @attribute {Boolean} focustrap
  * Determines if focus traversal can move above this view or not. The default 
  * is undefined which is equivalent to false. Can be ignored using a key 
...
/**
  ...
  * Determines if focus traversal can move above this view or not. The default 
  * is undefined which is equivalent to false. This is the same as focustrap 
  * except it can't be ignored using a key modifier.
  */

/**
  * @attribute {Boolean} maskfocus
  * Prevents focus from traversing into this view or any of its subviews. The 
...
/**
  ...
  * Indicates if the focus embellishment should be shown for this view or not 
  * when it has focus.
  */

/**
  * @attribute {String} cursor
  * Determines what cursor to show when moused over the view. Allowed values: 
  * 'auto', 'move', 'no-drop', 'col-resize', 'all-scroll', 'pointer', 
  * 'not-allowed', 'row-resize', 'crosshair', 'progress', 'e-resize', 
  * 'ne-resize', 'default', 'text', 'n-resize', 'nw-resize', 'help', 
  * 'vertical-text', 's-resize', 'se-resize', 'inherit', 'wait', 'w-resize',
  * 'sw-resize'. Defaults to undefined which is equivalent to 'auto'.
  */

/**
  * @attribute {Array} boxshadow
  * Sets a shadow around this view. In the array, index 0 is the horizontal 
  * shadow offset, index 1 is the vertical shadow offset, index 2 is the blur 
  * amount, index 3 is the spread and index 4 is the color. Each entry in the 
  * array is optional and the values default to: [0, 0, 4, 0, 'rgba(0,0,0,0.5)']
  *
  */

/**
  * @attribute {Array} gradient
  * Sets a linear gradient fill on the view. The value at index 0 is the 
  * gradient angle. The remaining entries are the color stops. Each color 
  * stop is a color value plus an optional location percentage. If no stops 
  * are provided the current bgcolor and color will be used. If those do 
  * not exist transparent will be used.
  */

/**
  * @attribute {Array} subviews
  * The array of child dr.Views for this view. Should be accessed through 
  * the getSubviews method.
  */

/**
  * @attribute {Array} layouts
  * The array of child dr.Layouts for this view. Should be accessed through 
...
/**
  ...
  * @private
  * The sum of borderleft, borderright, paddingleft and paddingright.
  */

/**
  * @attribute {Number} __fullBorderPaddingHeight
  * @private
  * The sum of bordertop, borderbottom, paddingtop and paddingbottom.
  */

/**
  * @attribute {dr.AutoPropertyLayout} __autoLayoutwidth
  * @private
  * A reference to the layout used for auto widths.
  */

/**
  * @attribute {dr.AutoPropertyLayout} __autoLayoutheight
  * @private
  * A reference to the layout used for auto heights.
  */

/**
  * @attribute {Boolean} __isPercentConstraint_x
  * @private
  * True if a percent constraint is used for the x attribute.
  */

/**
  * @attribute {Boolean} __isPercentConstraint_y
  * @private
  * True if a percent constraint is used for the y attribute.
  */

/**
  * @attribute {Boolean} __isPercentConstraint_width
  * True if a percent constraint is used for the width attribute.
  */

/**
  * @attribute {Boolean} __isPercentConstraint_height
  * True if a percent constraint is used for the height attribute.
  */

/**
  * @attribute {Boolean} __lockRecalc
  * Used to prevent recacalcuation of border, padding and corner radius when 
  * setting them via set_border, set_padding or set_cornerradius.
  */
...
/**
  ...
  * If true, this view recieves mouse events. Automatically set to true when 
  * an onclick/mouse* event is registered for this view.
  */
...
/**
  ...
  * If true, this view clips to its bounds and provides scrolling to see 
  * content that overflows the bounds
  */
...
/**
  ...
  *
  * *(note: setting a 'z' value for a view implicitly sets its parent's 
...
/**
  ...
  * When this value is set, items further from the camera will appear smaller, 
  * and closer items will be larger.
  */
...
/**
  ...
  * Cursor that should be used when the mouse is over this view, can be any 
  * CSS cursor value. Only applies when clickable is true.
  */

/**
  * @attribute {String} [boxshadow]
  * Drop shadow using standard CSS format (offset-x offset-y blur-radius 
  * spread-radius color). For example: "10px 10px 5px 0px  *888888".
  */
...
define(function(require, exports, module) {
    ...
    module.exports = dr.View = new JS.Class('View', dr.Node, {
        ...
        extend: {
            getLayoutHint: function(hints, layoutName, hintName) {
                if (hints) {
        ...
        getLayoutHint: function(layoutName, hintName) {
            return dr.View.getLayoutHint(this.layouthint, layoutName, hintName);
        },
        ...
        moveToFront: function() {
            if (this.sprite.moveToFront()) {
                ...
                if (parent) {
                    var svs = parent.subviews, i = svs.length;
                    while (i) {
                        if (svs[--i] === this) {
                            svs.splice(i, 1);
                            svs.push(this);
        ...
        __isPointVisible: function(x, y) {
            ...
            if (this.rectContainsPoint(x, y, 0, 0, this.width * effectiveScale, this.height * effectiveScale)) {
                var p = this.parent, pScale, parentSprite;
                
                // Allow for subviews outside the simple bounds of ancestor
                // views that are not clipped. See DREEM-104070789
                var v = this;
                while (p && !(p.clip || p.scrollable)) {
                    // Adjust x and y as we skip ancestor views.
                ...
                if (p) {
                    parentSprite = p.sprite;
                    pScale = p.__effectiveScale;
                    return p.__isPointVisible(
                        x + (v.x - parentSprite.getScrollX()) * pScale, 
                        y + (v.y - parentSprite.getScrollY()) * pScale
                    );
                }
                return true;
            }
        ...
        __layoutDomainInUseForView: function(domain) {
            // Check for management by parent (sibling) layouts of this view
            var parent = this.parent,
                layouts = parent.getLayouts(),
                i = layouts.length, layout;
            while (i) {
                layout = layouts[--i];
                if (layout.__layoutDomainInUseForView(domain, this)) return true;
            }
            
            // Checked for management by child layouts of this view
        ...
        __layoutDomainUsedByHintForView: function(domain) {
            // Check for management by parent (sibling) layouts of this view
            var parent = this.parent,
                layouts = parent.getLayouts(),
                i = layouts.length, layout;
            while (i) {
                layout = layouts[--i];
                if (layout.hasSubview(this) && layout.__layoutDomainUsedByHintForView(domain, this)) return true;
            }
        ...
        __shouldRestoreForLayoutHint: function(domain, from, to) {
            ...
            while (i) {
                layout = layouts[--i];
                if (layout.hasSubview(this) && layout.__shouldRestoreForLayoutHint(domain, from, to)) return true;
            }
